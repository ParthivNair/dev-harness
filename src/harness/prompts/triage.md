# Backlog triage rubric

Triage the open queued backlog below and return **structured judgements only**
...
1. **severity** — `high` | `med` | `low`.
   - `high`: a correctness/safety/data-loss bug, a broken build/test, or a violation of the
     project's core architectural or safety invariants.
...
3. **depends_on** — issue numbers this one must follow (it references a dependency in its body,
   or is logically blocked by another queued issue). The claimer will not start an issue while
   any dependency is still open, so be accurate.
